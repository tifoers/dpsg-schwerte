...
$category_ids = get_post_meta( get_the_ID(), 'dpsg_dahl_news_category_id', true );
...
if($posts):
  ...
  ?>
    <div class="grid-x feature-boxes">
      <div class="row archive" id="frontPageNews">
        <div class="columns small-12">
          <?php if ($category_ids = get_post_meta( get_the_ID(), 'dpsg_dahl_news_title', true ) ): ?>
            <h2><?php echo $category_ids ?>:</h2>
          <?php else: ?>
            <h2>Das Neuste aus unserem Stamm:</h2>
          <?php endif; ?>


        </div>
        <?php foreach ($posts as $post): setup_postdata($post); ?>
          <div id="post-<?php the_ID(); ?>" <?php post_class('cell small-6 medium-4'); ?>>
            <header>
